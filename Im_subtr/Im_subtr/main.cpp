...
int main(int argc, char** argv) {
	...
	while (capVideo.isOpened() && chCheckForEscKey != 27) {
		...
		if (countFrames >= 20)
		{
			cv::Mat diff_hog = imgBack.clone();
			cv:absdiff(imgBack, imgFrame2Copy, diff_hog);
			vector<Rect> found = detector.detect(diff_hog);
			cv::Mat frame = imgFrame2.clone();
			if (!found.empty())
			{
